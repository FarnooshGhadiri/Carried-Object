function display_hypo_rect_mask(edge_map,vote_map,hypo_list,score_list,...
    bbox,vote_record,valid_vote_idx,testpos,hypo_mask)
%display_hypo_rect_mask(edge_map,vote_map,hypo_list,score_list,bbox,
...
imshow(edge_map);
%title(sprintf('max voting score:%.4f',max_score));
title(sprintf('Probable person contour'));
hold on;
colors=jet(64);
...
for hypo=1:nb_hypo
    % plot voters
    idx_code_curhypo= vote_record(hypo).voter_id;
...
if(exist('hypo_mask','var') && ~isempty(hypo_mask) && ~isempty(score_list))
    ...
    if(~isempty(score_list))
        ...
        imshow(hypo_mask(:,:,m_id));
        %axis image;
        title('mask of hypothesis with max score');
    end
end

Remain_contour=(edge_map-(edge_map.*hypo_mask));
if(~isempty(Remain_contour))
    plot_idx    = plot_idx+1;
    subplot(1,nb_plot, plot_idx);
    imshow(Remain_contour(:,:));
    %axis image;
    hold on;
    %plot(hypo_list(:,1), hypo_list(:,2), 'g+');
    title('Probable carried object contour');
end
